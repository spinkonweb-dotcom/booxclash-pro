...
def calculate_week_dates(start_date_str: str, week_num: int) -> Dict[str, str]:
    ...
    Returns a dictionary with formatted display strings and raw ISO dates.
    """
    try:
...
async def generate_scheme_with_ai(
    ...
    if not syllabus_data or not isinstance(syllabus_data, list):
        print(f"⚠️ [Scheme Generator] CRITICAL: No syllabus data found.")
        return [] 
    ...
    if not term_syllabus_data: return []
    ...
    2. **Content Details (STRICT)**: 
       ...
       - Include specific examples in text (e.g., "234,567 = 200,000 + ...").
    ...
    4. **References**:
       - Cite "Zambian Syllabus Grade {grade}" and "Pupil's Book".
    ...
    [
      {{
        ...
        "outcomes": ["Learners should be able to...", "Learners should be able to..."],
        "references": ["Zambian Syllabus"]
      }}
    ]
    """
    ...
    try:
        ...
        # ✅ 5. POST-PROCESSING: INJECT PYTHON CALCULATED DATES
        cleaned_data = []
        for i, item in enumerate(data):
            ...
            # Update the 'week' string to include the range automatically
            # This fixes the NaN error on frontend
            item['week'] = f"Week {week_num} {date_info['range_display']}"
            
            # Inject raw keys for safety
...
# =====================================================
# 3. DETAILED LESSON PLANNER (Optimized for Points/Summary)
# =====================================================
# =====================================================
