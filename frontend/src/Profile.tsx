...
export default function Profile() {
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br ${theme.primary} p-4 relative`}>
      {/* Theme Selector */}
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-8 pt-16">
          ...
          <p className={`text-lg ${theme.lightText}`}>Manage your account settings and information</p>
        </div>

        <div className="grid lg:grid-cols-3 gap-6">
          {/* Profile Overview Card */}
          <div className={`lg:col-span-2 ${theme.cardBg} backdrop-blur-sm rounded-xl shadow-lg p-6 border ${theme.border}`}>
            <div className="flex flex-col md:flex-row items-start gap-6">
              ...
              <div className="flex flex-col items-center space-y-4">
                <div className="relative">
                  <img
                    ...
                    alt="Profile"
                    className="w-32 h-32 rounded-full border-4 border-gradient-to-r shadow-lg object-cover"
                ...
                <div className="w-full max-w-xs">
                  ...
                  <label
                    ...
                    className={`w-full flex items-center justify-center gap-2 ${theme.accent} ${theme.accentHover} text-white px-4 py-2 rounded-lg font-medium transition-all duration-300 cursor-pointer hover:scale-105`}
                  >
                    <Camera size={16} />
                    Choose Photo
                  </label>
                  ...
                  {selectedFile && (
                    <div className="mt-3 space-y-2">
                      ...
                      <button
                        ...
                        disabled={loading}
                        className={`w-full ${theme.accent} ${theme.accentHover} text-white px-4 py-2 rounded-lg font-medium transition-all duration-300 hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed`}
                      >
              ...
              <div className="flex-1 space-y-4">
                <div className="flex items-center gap-3 mb-4">
                  <h2 className={`text-2xl font-bold ${theme.text}`}>{user.name}</h2>
                ...
                {user.role === 'student' && (
                  <div className="mt-6 pt-4 border-t border-gray-200">
                    <h3 className={`text-lg font-semibold ${theme.text} mb-3`}>Student Information</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {user.curriculum && (
                        <div className="flex items-center gap-3">
          ...
          {(user.role === 'parent' || user.role === 'school') && user.subscription && (
            <div className={`${theme.cardBg} backdrop-blur-sm rounded-xl shadow-lg p-6 border ${theme.border}`}>
              <div className="flex items-center gap-3 mb-4">
                <Crown className={theme.text} size={24} />
                <h3 className={`text-xl font-bold ${theme.text}`}>Subscription</h3>
              </div>

              <div className="space-y-4">
                <div>
                  <p className="text-xs text-gray-500 uppercase tracking-wide">Plan</p>
                  <p className="text-gray-800 font-bold capitalize">{user.subscription.plan.replace('_', ' ')}</p>
                </div>
                ...
                {typeof user.subscription.childrenLimit === 'number' && (
                  <div>
                    <p className="text-xs text-gray-500 uppercase tracking-wide">Children Limit</p>
                    <p className="text-gray-800 font-bold">{user.subscription.childrenLimit}</p>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className={`lg:col-span-${(user.role === 'parent' || user.role === 'school') && user.subscription ? '2' : '1'} ${theme.cardBg} backdrop-blur-sm rounded-xl shadow-lg p-6 border ${theme.border}`}>
            <div className="flex items-center gap-3 mb-4">
              <Lock className={theme.text} size={24} />
              <h3 className={`text-xl font-bold ${theme.text}`}>Change Password</h3>
            </div>
