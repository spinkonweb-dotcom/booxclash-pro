...
type ItemCategory = 'desert' | 'arctic';
...
const ALL_GAME_ITEMS: GameItem[] = [
  // Desert Items
  { id: "camel", name: "Camel", emoji: "🐫", category: "desert" },
  { id: "cactus", name: "Cactus", emoji: "🌵", category: "desert" },
  { id: "lizard", name: "Lizard", emoji: "🦎", category: "desert" },
  { id: "scorpion", name: "Scorpion", emoji: "🦂", category: "desert" },
  { id: "fennec", name: "Fennec Fox", emoji: "🦊", category: "desert" },

  // Arctic Items
  { id: "polarBear", name: "Polar Bear", emoji: "🐻‍❄️", category: "arctic" },
  { id: "penguin", name: "Penguin", emoji: "🐧", category: "arctic" },
  { id: "seal", name: "Seal", emoji: "🦭", category: "arctic" },
  { id: "wolf", name: "Arctic Wolf", emoji: "🐺", category: "arctic" },
  { id: "reindeer", name: "Reindeer", emoji: "🦌", category: "arctic" },
];

const BINS: { category: ItemCategory; emoji: string; label: string; color: string; highlight: string; }[] = [
  { category: "desert", emoji: "🌵", label: "Desert", color: "bg-orange-600", highlight: "border-orange-400" },
  { category: "arctic", emoji: "❄️", label: "Arctic", color: "bg-cyan-600", highlight: "border-cyan-400" },
];
...
const HabitatMatchGame: React.FC<LessonProps> = ({ onComplete }) => {
  // --- STATE ---
  const [unsortedItems, setUnsortedItems] = useState<GameItem[]>([]);
  const [sortedItems, setSortedItems] = useState<{ [key in ItemCategory]: GameItem[] }>({ desert: [], arctic: [] });
  ...
  if (isGameComplete) {
    return (
      <div className="p-6 text-center flex flex-col items-center justify-center h-full text-white">
        ...
        <p className="text-lg mb-6">You know where everyone lives!</p>
        <button
          onClick={onComplete}
  ...
  return (
    <div className="p-4 flex flex-col h-full text-white overflow-hidden">
      <h3 className="text-2xl font-bold text-center mb-4">Habitat Match</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {BINS.map((bin) => (
          <div
            ...
            className={`min-h-[250px] rounded-xl p-4 flex flex-col items-center justify-start transition-all duration-200 border-4 border-dashed ${bin.color} ${
              dragOverBin === bin.category ? `${bin.highlight} scale-105 shadow-lg` : "border-white/20"
            }`}
          >
            {/* Bin Icon */}
            <div className={`w-full flex items-center justify-center mb-2`}>
              <span className="text-8xl">{bin.emoji}</span>
            </div>
            
            <p className="text-2xl font-bold">{bin.label}</p>
            
            {/* Show items already sorted into this bin */}
            <div className="flex flex-wrap justify-center mt-2 gap-1 text-2xl flex-grow">
...
export default HabitatMatchGame;