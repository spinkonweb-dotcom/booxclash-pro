...
type ItemCategory = 'healthy' | 'sick';
...
const ALL_GAME_ITEMS: GameItem[] = [
  // Healthy
  { id: "apple", name: "Apple", emoji: "🍎", category: "healthy" },
  { id: "sleep", name: "Sleep", emoji: "😴", category: "healthy" },
  { id: "broccoli", name: "Broccoli", emoji: "🥦", category: "healthy" },
  { id: "soap", name: "Soap", emoji: "🧼", category: "healthy" },
  { id: "exercise", name: "Exercise", emoji: "🏃", category: "healthy" },
  { id: "tissue", name: "Tissue", emoji: "🧻", category: "healthy" }, // Using a tissue is a healthy action
  
  // Sick / Unhealthy
  { id: "cough", name: "Cough", emoji: "🤧", category: "sick" },
  { id: "germ", name: "Germs", emoji: "🦠", category: "sick" },
  { id: "spoiled", name: "Spoiled Food", emoji: "🍲", category: "sick" }, // (Using 'smelly' emoji as proxy)
  { id: "trash", name: "Trash", emoji: "🗑️", category: "sick" },
  { id: "soda", name: "Soda", emoji: "🥤", category: "sick" },
  { id: "candy", name: "Candy", emoji: "🍬", category: "sick" },
];

const BINS: { category: ItemCategory; emoji: string; label: string; color: string; highlight: string; }[] = [
  { category: "healthy", emoji: "💪", label: "Healthy", color: "bg-green-600", highlight: "border-green-400" },
  { category: "sick", emoji: "🤢", label: "Sicky", color: "bg-purple-600", highlight: "border-purple-400" },
];
...
const WhatMakesUsSickGame: React.FC<LessonProps> = ({ onComplete }) => {
  // --- STATE ---
  const [unsortedItems, setUnsortedItems] = useState<GameItem[]>([]);
  const [sortedItems, setSortedItems] = useState<{ [key in ItemCategory]: GameItem[] }>({ healthy: [], sick: [] });
  ...
  if (isGameComplete) {
    return (
      <div className="p-6 text-center flex flex-col items-center justify-center h-full text-white">
        ...
        <h2 className="text-3xl font-bold">Health Hero!</h2>
        <p className="text-lg mb-6">You know what's healthy and what's not!</p>
        <button
          onClick={onComplete}
  ...
  return (
    <div className="p-4 flex flex-col h-full text-white overflow-hidden">
      <h3 className="text-2xl font-bold text-center mb-4">Good Choice or Bad Choice?</h3>
...
export default WhatMakesUsSickGame;
